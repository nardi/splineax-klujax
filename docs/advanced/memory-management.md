...
The id is a content hash of the matrix the handle names (dtype, sparsity pattern,
values), so a handle names a matrix, not a mutable slot. Two things follow from
this. First, a handle can never be handed the wrong matrix. If the slot it named
is gone, or was re-keyed by a `refactor`, a call rebuilds the original matrix from
the arrays the token carries. That costs a rebuild but never returns a wrong
answer. Second, identical matrices hash equal, so factoring the same matrix twice
reuses one factorization (dedup). A `refactor` re-keys to the new values rather
than overwriting the old handle, so a stale alias of the old token stays correct.

...
A numeric handle is a bare pointer and does not record whether it was built from real or complex values, but KLU needs the matching entry point (`klu_rcond` or `klu_z_rcond`). So `rcond` takes the dtype the factorization was built with. `condest` gets it from `Ax` and needs no such argument.

...
class RebuildReason(enum.IntEnum):
    ...
    carries. That is always correct but costs the rebuild. NONE means the
    resident factorization was used directly (no rebuild). The status variants
    (solve_with_numeric_with_status, refactor_with_status, ...) report one value
    per numeric handle.

    Values mirror the RebuildReason enum in klujax.cpp.
    """
...
def solve_with_numeric_with_status(
    ...
    Returns the solution and, per numeric handle, a RebuildReason: NONE when the
    resident factorization was used, or EVICTED / FREED / SUPERSEDED / UNKNOWN when
    the handle was gone and rebuilt from the token's carried arrays (still correct,
    just costlier). SUPERSEDED means a refactor elsewhere re-keyed this handle -- the
    classic "stale alias" case. The rebuild array is an ordinary output, so it can be
    branched on under jax.jit.
...
def general_vmap_with_numeric_status(
    ...
    **params: Any,  # noqa: ANN401
    ...
    # rebuild array (shape of numeric). x batches like the plain solve; rebuild
    # batches with the numeric handle when that is the vectorized axis.
    Ai, Aj, Ax, symbolic, numeric, b = vector_arg_values
    aAi, aAj, aAx, asymbolic, anumeric, ab = batch_axes

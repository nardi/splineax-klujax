...
def test_complex_numeric_real_rhs_dispatches_complex():
    ...
    The KLU entry point must follow the factorization's dtype, not b's; otherwise a
    real solve would reinterpret a complex numeric's memory. The answer must match a
    dense complex solve.
    """
...
def test_rebuild_reason_visible_under_jit():
    ...
    _log_and_test_equality(jsp.linalg.solve(_dense(Ai, Aj, Ax, n_col, np.float64), b), x)
